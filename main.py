...
#*********************************************************************************************************************
    def atualizar_usuario_no_bd(self):
        ...
        usuario_endereco = self.txt_endereco.text() or "Não Cadastrado"
        usuario_numero = self.txt_numero.text() or "Não Cadastrado"
        usuario_cidade = self.txt_cidade.text() or "Não Cadastrado"
        usuario_bairro = self.txt_bairro.text() or "Não Cadastrado"
        usuario_complemento = self.txt_complemento.text() or "Não Cadastrado"
        usuario_email = self.txt_email.text() or "Não Cadastrado"
        usuario_data_nascimento = self.txt_data_nascimento.text() or "Não Cadastrado"
        usuario_rg = self.txt_rg.text() or "Não Cadastrado"
        usuario_cpf = self.txt_cpf.text() or "Não Cadastrado"
        usuario_cnpj = self.txt_cnpj.text() or "Não Cadastrado"
        usuario_cep = self.txt_cep.text() or "Não Cadastrado"
        usuario_estado = self.perfil_estado.currentText()
        ...
        if tem_imagem:
            ...
            if pixmap:
                ...
                imagem_bytes = bytes(buffer.data())
                usuario_imagem = base64.b64encode(imagem_bytes).decode("utf-8")
        ...
        sql = """
            UPDATE users 
            SET Nome=?, Usuário=?,Telefone=?, Endereço=?, Cidade=?,Bairro=?, Número=?, Complemento=?, 
            Email=?, "Data de Nascimento"=?, RG=?, CPF=?,CNPJ=?, CEP=?, Estado=?, Senha=?,
            "Confirmar Senha"=?,Acesso=?
            WHERE id=?
        """

        # Valores para substituir os placeholders
        valores = (usuario_nome, usuario_usuario,usuario_telefone, usuario_endereco, usuario_bairro,usuario_cidade,usuario_numero, usuario_complemento,
                usuario_email, usuario_data_nascimento, usuario_rg, usuario_cpf, usuario_cnpj,usuario_cep,
        ...
        if usuario_imagem:
            sql = """
                UPDATE users 
                SET Nome=?, Usuário=?,Telefone=?, Endereço=?, Cidade=?,Bairro=?, Número=?, Complemento=?, 
                Email=?, "Data de Nascimento"=?, RG=?, CPF=?, CNPJ=?, CEP=?, Estado=?, Senha=?, Imagem=?,
                "Confirmar Senha"=?, Acesso=?
                WHERE id=?
            """
            # Adicione a imagem aos valores
            valores = (usuario_nome, usuario_usuario,usuario_telefone, usuario_endereco,usuario_cidade,usuario_bairro, usuario_numero, usuario_complemento,
                    usuario_email, usuario_data_nascimento, usuario_rg, usuario_cpf,usuario_cnpj, usuario_cep,
