...
class PrimeiroAcesso(QDialog):  
    ...
    def _criar_olho_senha(self, line_edit: QLineEdit, object_name: str):
        ...
        # ======================================
        # Offset do posicionamento por tema
        # (aqui você controla: -11 vs -15)
        # ======================================
        offset_x = 8 if tema == "classico" else 11
        
        offset_y = 2 if tema == "classico" else 4
        

        def posicionar():
            # garante que pega o tamanho real já calculado pelo layout
            x = line_edit.width() - btn_size - offset_x
            y = (line_edit.height() - btn_size) // offset_y
            btn.move(x, y)
